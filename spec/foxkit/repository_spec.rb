...
describe Foxkit::Repository do
  context "when passed a string containg a slash" do
    before do
      @repository = Foxkit::Repository.new("sferik/octokit")
    end
    ...
    it "renders url as string" do
      expect(@repository.url).to eq('https://gitlab.com/sferik/octokit')
    end
  end
  ...
  describe ".path" do
    ...
    context "with repository id" do
      it "returns theu url path" do
        repository = Foxkit::Repository.new(12345)
  ...
  describe "self.path" do
    it "returns the api path" do
      expect(Foxkit::Repository.path('sferik/octokit')).to eq 'repos/sferik/octokit'
      expect(Foxkit::Repository.path(12345)).to eq 'repositories/12345'
    end
  end
  ...
  context "when passed a hash" do
    it "sets the repository name and username" do
      repository = Foxkit::Repository.new({:username => 'sferik', :name => 'octokit'})
      expect(repository.name).to eq("octokit")
      expect(repository.username).to eq("sferik")
    end
  end
  ...
  context "when passed a Repo" do
    it "sets the repository name and username" do
      repository = Foxkit::Repository.new(Foxkit::Repository.new('sferik/octokit'))
      expect(repository.name).to eq("octokit")
      expect(repository.username).to eq("sferik")
      expect(repository.url).to eq('https://gitlab.com/sferik/octokit')
    end
  ...
  context "when given a URL" do
    it "sets the repository name and username" do
      repository = Foxkit::Repository.from_url("https://gitlab.com/sferik/octokit")
      expect(repository.name).to eq("octokit")
      expect(repository.username).to eq("sferik")
    end
  end
